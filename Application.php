...
{
	/**
	 * @var array Application instance list
	 */
	...
	public function __construct($name = null)
	{
		if (!count(self::$_instances))
			$this->_instanceName = self::$_defaultInstanceName;
		else
			$this->_instanceName = $name = $name ? $name : str_replace(__NAMESPACE__, '', __CLASS__);
	...
	public static function config($configName = null)
	{
		try
		{
			...
			{
				$bag = self::instance()->component('config')->getConfigBag($configName);
				return $bag;
			}
		}
		catch (Exception\ComponentNotFoundException $e)
